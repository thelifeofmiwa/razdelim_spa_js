...
export function useResult() {
    ...
    const formVisible = ref(false); //реактивная переменная, отвечающая за видимость формы конкретизации долгов

    const router = useRouter(); // переменная для работы с Vue Router

    const debts = computed(() =>
        // переменная с посчитанными данными долгов, округлёнными до 2 чисел после запятой
        consolidateDebts(
    ...
    function showAndHide() {
        // метод для отображения подробностей долга
    ...
    function countAgain() {
        // метод для работы с роутером ; возвращает на страницу persons и очищает хранилище
        router.push({ path: "/persons" });
        store.$reset();
    }

    function addDebt(person, personName, amount) {
        // метод для добавления долга
        const existingDebt = person.debts.find(
            //проверка на наличие долга перед человеком; если долга нет - создаётся новый объект, если есть - суммируется со старым
    ...
    function calculateDebts(persons, products) {
        //первичный рассчёт долгов персон
    ...
    function consolidateDebts(persons) {
        // метод для вычета взаимных долгов
